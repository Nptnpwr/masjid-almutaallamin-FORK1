        ...
        <form action="{{ route('admin.donations.update', $donation->id) }}" method="POST">
            ...
            <div class="mb-3">
                ...
                <input type="text" 
                       ...
                       id="bank" 
                       name="bank" 
                       value="{{ old('bank', $donation->bank) }}"
                       placeholder="Contoh: Bank BCA, Bank Mandiri, BSI"
                       required>
                @error('bank')
            ...
            <div class="mb-3">
                <label for="nama_pemilik" class="form-label">Nama Pemilik Rekening <span class="text-danger">*</span></label>
                <input type="text" 
                       class="form-control @error('nama_pemilik') is-invalid @enderror" 
                       id="nama_pemilik" 
                       name="nama_pemilik" 
                       value="{{ old('nama_pemilik', $donation->nama_pemilik) }}"
                       placeholder="Contoh: Masjid Al Muta'allimin"
                       required>
                @error('nama_pemilik')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
